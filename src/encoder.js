...
function encodeInputs(inputs, add_address_to_previous_output_index) {
  ...
  inputs.forEach((input, index) => {
    //Write reversed hash
    offset += Buffer.from(input.previous_output.hash, "hex")
      .reverse()
      .copy(buffer, offset);
    //Index
    ...
    if (add_address_to_previous_output_index !== undefined) {
      if (index == add_address_to_previous_output_index) {
        ...
        if (
          ...
          input.previous_output.script.match(lockregex)
        ) {
          let locktime = lockregex.exec(
            input.previous_output.script.match(lockregex)[0]
          )[1];
          offset = writeScriptLockedPayToPubKeyHash(
          ...
          if (Script.hasAttenuationModel(input.previous_output.script)) {
            let params = Script.getAttenuationParams(
              input.previous_output.script
            );
            offset = writeAttenuationScript(
              params.model,
              params.hash !==
            ...
            if (Script.isP2SH(input.previous_output.script)) {
              ...
              offset += bufferutils.writeVarInt(
                buffer,
              ...
              if (input.previous_output.script) {
                let script_buffer = Script.fromASM(
                  input.previous_output.script
                ).toBuffer();
                offset += bufferutils.writeVarInt(
                  buffer,
                ...
                offset = writeScriptPayToPubKeyHash(
                  input.previous_output.address,
                  buffer,
                  offset
                );
  ...
  outputs.forEach(output => {
    ...
    if (output.script_type) {
      switch (output.script_type) {
        ...
        case "op_return":
          ...
          break;
        case "lock":
          let locktime_le_string = parseInt(output.locktime)
            .toString(16)
            .replace(/^(.(..)*)$/, "0$1")
            .match(/../g)
            .reverse()
          ...
          break;
        case "attenuation":
          let model = Script.deserializeAttenuationModel(
            output.attenuation.model
          );
          if (output.attenuation.height_delta > 0)
            model = Script.adjustAttenuationModel(
              model,
              output.attenuation.height_delta
            );
          offset = writeAttenuationScript(
            Script.serializeAttenuationModel(model),
            output.attenuation.from_tx,
            output.attenuation.from_index,
            output.address,
            buffer,
            offset
          );
    ...
    switch (output.attachment.type) {
      ...
      case Constants.ATTACHMENT.TYPE.MST:
        switch (output.attachment.status) {
          case Constants.MST.STATUS.REGISTER:
            offset = encodeAttachmentAssetIssue(
              buffer,
              offset,
              output.attachment
            );
          ...
          case Constants.MST.STATUS.TRANSFER:
            offset = encodeAttachmentMSTTransfer(
              buffer,
              offset,
              output.attachment.symbol,
      ...
      case Constants.ATTACHMENT.TYPE.MESSAGE:
        offset = encodeAttachmentMessage(
          buffer,
          offset,
          output.attachment.message
        );
        break;
      ...
      case Constants.ATTACHMENT.TYPE.MIT:
        switch (output.attachment.status) {
          case Constants.MIT.STATUS.REGISTER:
            offset = encodeAttachmentMITRegister(
              buffer,
              offset,
              output.attachment.symbol,
              output.attachment.content,
              output.attachment.address
            );
            break;
          case Constants.MIT.STATUS.TRANSFER:
            offset = encodeAttachmentMITTransfer(
              buffer,
              offset,
              output.attachment.symbol,
              output.attachment.address
...
function encodeAttachmentAssetIssue(buffer, offset, attachment_data) {
  ...
  offset = bufferutils.writeUInt64LE(
    buffer,
    attachment_data.max_supply,
    offset
  );
  //Encode precision
  ...
  offset = buffer.writeUInt8(
    attachment_data.secondaryissue_threshold
      ? attachment_data.secondaryissue_threshold
      : 0,
    offset
  );
...
function writeScriptPayToScriptHash(scripthash, buffer, offset) {
  ...
  offset += Buffer.from(base58check.decode(scripthash, "hex").data, "hex").copy(
    buffer,
    offset
  );
...
function writeScriptPayToPubKeyHash(address, buffer, offset) {
  ...
  //Write previous output address
  offset = buffer.writeUInt8(20, offset); //Address length
  offset += Buffer.from(base58check.decode(address, "hex").data, "hex").copy(
    buffer,
    offset
  );
  //Static transfer stuff
...
function writeAttenuationScript(
  ...
  offset += bufferutils.writeVarInt(
    buffer,
    26 + attenuation_string.length + 40,
    offset
  );
...
function fromBuffer(tx, buffer, network) {
  ...
  for (i = 0; i < output_length; ++i) {
    ...
    if (Script.hasAttenuationModel(output.script)) {
      ...
      output.address = Script.getAddressFromOutputScript(
        output.script,
        network
      );
    }
